...
## 3. Дать участнице ручной уровень (Бриллиант, Легенда и др.)

«Уровень» (🌸/🔥/💚/👑/💎) — вычисляемый через `getEffectiveMonths(subscription_started_at, subscription_plan)` в `src/lib/webinars.ts`. В БД отдельного поля нет.

Пороги (из `src/lib/webinars.ts`):

| Уровень | Эффективных месяцев |
|---|---|
| 🌸 Новенькая | < 3 |
| 🔥 Вошла во вкус | 3–5 |
| 💚 Уже своя | 6–8 |
...
Бонус: `subscription_plan = 'halfyear'` даёт **+6 месяцев** к расчёту.

### Как дать уровень — установить subscription_started_at в прошлое

Пример: дать Бриллиант (12+ мес от сегодня):
```sql
UPDATE members
SET subscription_started_at = now() - interval '13 months'
WHERE email = 'user@example.com';
```

Пример: дать «Вошла во вкус» (3+ мес):
```sql
UPDATE members
SET subscription_started_at = now() - interval '3 months' - interval '1 week'
WHERE email = 'user@example.com';
```

### Что НЕ нужно делать

- **Не обновлять `tariff = 'Бриллиант'`** — поле существует, но в формуле уровня не участвует (legacy)
- **Не поднимать `is_manual_subscription = true`** если участница платит через CloudPayments — это сломает продление `subscription_ends_at` при платежах

### Что произойдёт дальше

- При следующем платеже webhook не перезапишет `subscription_started_at` (фикс от 2026-06-08)
- Уровень останется, пока кто-то снова не обновит `subscription_started_at` вручную

---

## 4. Откат ручного апгрейда (убрать VIP)
...
## 8. Активация дня марафона вручную
