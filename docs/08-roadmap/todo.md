...
- [ ] **R96** (высокий) — Rule #11 в `src/proxy.ts:213` — кураторская проверка в middleware. `supabase.from('members').select('role').eq('id', user.id)` — нарушение правила #11: в этом проекте `auth.users.id ≠ members.id`. Если они расходятся — curator'а пропустит или заблокирует неверно. Исправить на email lookup. Найдено в разведке публичного сайта, относится к клубной auth. Источник: `src/proxy.ts:213`.
...
- [ ] **R102** (средний) — `/recipes` скрыт из PublicNav. В `src/components/public/PublicNav.tsx` пункт «Рецепты» закомментирован, но страница есть в sitemap и в PublicFooter. Решить: раскрыть пункт в хедере (если контент готов) или убрать из sitemap и footer до наполнения. Источник: `src/components/public/PublicNav.tsx`.

### Публичный сайт — блог (разведка 2026-05-23)

- [ ] **R103** (средний-высокий) — Баг в sitemap: блог-секция включает рецепты с неверными URL. `sitemap.ts` запрашивает все `blog_posts` с `is_published=true`, включая рецепты. `getArticleUrl()` возвращает `/blog/{slug}` для постов с категорией из BLOG_CATEGORIES (не SILO) → 404-URL в sitemap. Яндекс/Google снижают рейтинг сайта с мёртвыми URL. Фикс: добавить фильтр `.not('category', 'in', RECIPES_CONFIG_KEYS)` в запрос блог-секции, либо `.neq('type', 'recipe')`. **Требует фикса до запуска активного продвижения.** Источник: `src/app/public-site/sitemap.ts`.
