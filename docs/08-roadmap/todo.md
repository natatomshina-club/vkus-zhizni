...
- [x] **Найти источник писем «Доступ в клуб приостановлен»** ✅ 2026-06-06 — источник: Vercel cron legacy deployment. Отключён 2026-06-05. Подробнее: [[07-sessions/2026-06-06]].
...
- [x] **Серия писем «churn»** ✅ 2026-06-06 — реализован cron `expiry-followup` (5 шагов: дни 1/2/3/15/30). Модуль: [[06-operations/churn-series]].
...
- [x] **check-subscriptions: добавить email при истечении подписки** ✅ 2026-06-06 — реализован отдельным cron `expiry-followup`. Модуль: [[06-operations/churn-series]].
...
- [x] **R78** ✅ 2026-06-06 — добавлена проверка `kbju_manual` перед пересчётом КБЖУ в `POST /api/tracker/measurements`. Ручной override теперь сохраняется. Источник: `src/app/api/tracker/measurements/route.ts`.
...
- [x] **R121** ✅ 2026-06-08 — Баг сброса `subscription_started_at` при рекуррентных платежах. PAY-блок webhook перезаписывал поле при каждом платеже, сбрасывая уровень участницы к «Новенькой». Исправлено: webhook сохраняет существующее значение (`existing.subscription_started_at ?? new Date().toISOString()`). Коммиты `564a734`+`4802229`.

- [x] **R122** ✅ 2026-06-08 — `lesson_type` был управляющим флагом для рендера видео/текста в `CoursePageClient.tsx`; при сохранении `video_url` через PATCH-роут тип не обновлялся — видео не показывалось. Исправлено: рендер data-driven по наличию `videoId`/`textContent`; PATCH авто-выставляет `lesson_type='video'`. Коммит `4db5523`.

### R114 — Удалить Vercel project

Зайти на vercel.com → проект → Settings → Advanced → Delete Project.
DNS не указывает на Vercel (`club.nata-tomshina.ru` → Beget) — удаление безопасно.
Vercel cron'ы уже отключены (2026-06-05), но пока проект существует — риск случайного
re-deploy остаётся (например, при пуше в GitHub если не отвязан от Vercel).

### R115 — Ротейт Resend API ключа (после удаления Vercel)

После удаления Vercel project: выпустить новый ключ в `resend.com/api-keys` и обновить
`SMTP_PASS` в `/opt/supabase/docker/.env` + перезапустить `supabase-auth` контейнер.
Отзывает старый ключ, который знал Vercel. Не срочно — GoTrue OTP стабилен.

### R116 — Нормализовать TARIFF_LABELS, убрать дубликаты русских ключей
...
Связано: tech-debt про `tariff` vs `subscription_plan`.

### R117 — Потеря шага при пропуске cron (expiry-followup)

Условие `day === N` в `expiry-followup/route.ts` — точное совпадение дня.
Если cron не запустился (перезагрузка сервера, ошибка) — письмо за этот день
теряется насовсем. Особенно критично для day 15: пропуск = письмо «скоро марафон»
не придёт никогда.

Потенциальный фикс: заменить `day===N && step<N` на `day>=N && step<N`.
Тогда письмо отправится при первом запуске после пропуска.
Источник: `src/app/api/cron/expiry-followup/route.ts`. Связано: [[06-operations/churn-series]].

### R118 — is_manual_subscription не фильтруется в churn-серии

`expiry-followup` отправляет письма всем `expired`, включая VIP-участниц
с `is_manual_subscription = true`. Подписка истекла — вероятно они должны
получить уведомление, но текст «вернитесь» может быть неуместен.

Решить с Наташей: нужна ли отдельная коммуникация для VIP или текущее поведение ок.
Источник: `src/app/api/cron/expiry-followup/route.ts`. Связано: [[06-operations/churn-series]].

### R119 — NULL subscription_plan у 14 участниц

14 участниц с `subscription_plan = NULL` и `last_payment_amount = 1500`.
Функционально не сломаны (для `month` бонус = 0), но NULL вместо `'month'`.
Причина: ранние версии webhook не заполняли `plan` из `Data.Metadata`.
...
WHERE last_payment_amount = 1500 AND subscription_plan IS NULL;
```
Находка: [[05-infrastructure/_findings#2026-06-08--r119-null-subscription_plan]].

### R120 — RECURRENT-блок webhook — мёртвый код

CloudPayments присылает все транзакции как `OperationType='Payment'`.
Блок `OperationType === 'Recurrent'` в `payments/cloudpayments/route.ts`
никогда не получает управления на проде.

Не удалять: если CloudPayments изменит поведение — RECURRENT-блок станет активным
и содержит корректную защитную логику. Оставить как есть.
Находка: [[05-infrastructure/_findings#2026-06-08--r120-recurrent-блок]].
