...
### R-NEW-2 — Удалить Vercel project + опциональный ротейт Resend ключа

**Удаление Vercel project:** зайти на vercel.com → проект → Settings → Advanced → Delete Project.
...
**Опциональный ротейт Resend API ключа:** после удаления Vercel project выпустить новый
ключ в `resend.com/api-keys` и обновить `SMTP_PASS` в `/opt/supabase/docker/.env` +
перезапустить `supabase-auth` контейнер. Это отзывает старый ключ, который знал Vercel.
Не срочно — старый ключ работает и GoTrue OTP с ним стабилен.
