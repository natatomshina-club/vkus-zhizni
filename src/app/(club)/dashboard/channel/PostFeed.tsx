...
export default function PostFeed({ channel, memberId, isAdmin, isCurator, memberName, memberFullName }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Plates info strip — compact, no yellow banner */}
      {isPlates && (
        <div
          className="mx-4 mt-3 mb-0 flex items-center gap-1.5 shrink-0"
        ...
        >
          <button
            ...
            style={{
              ...
              fontFamily: 'var(--font-nunito)', fontSize: 12, color: 'var(--muted)',
              padding: '2px 4px',
            }}
          >
          ...
          {infoTooltip && (
            <div
              style={{
                ...
                fontSize: 12, color: 'var(--text)', fontFamily: 'var(--font-nunito)',
                boxShadow: '0 4px 16px rgba(0,0,0,0.1)',
                maxWidth: 240, lineHeight: 1.5, marginTop: 4,
              }}
            >
              📷 Фото удаляются автоматически через 72 часа
              <button
                onClick={() => setInfoTooltip(false)}
                style={{
                  display: 'block', marginTop: 6, fontSize: 11,
                  color: 'var(--pur)', background: 'none', border: 'none',
                  cursor: 'pointer', padding: 0, fontFamily: 'var(--font-nunito)',
                }}
              >
                Закрыть
              </button>
