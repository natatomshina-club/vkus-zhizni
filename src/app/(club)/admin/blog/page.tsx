...
interface BlogPost {
  ...
  created_at: string; category: string | null; widget_type: string | null
}
...
function ArticlesTab() {
  ...
  function startEdit(p: BlogPost) {
    ...
    setForm({ title: p.title, slug: p.slug, excerpt: p.excerpt ?? '', content: p.content ?? '',
      ...
      category: p.category ?? '', widget_type: p.widget_type ?? '' })
    setSaveErr('')
  }
    ...
    <div>
      ...
      {isFormOpen && (
        <form onSubmit={handleSave}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div><label style={labelStyle}>Рубрика</label>
...
type Tab = 'articles' | 'topics' | 'generator' | 'settings'
