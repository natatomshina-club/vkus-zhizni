...
export default function MembersPage() {
  ...
  async function handleAddMember(e: React.FormEvent) {
    ...
    setAddSaving(false)
    const d = await res.json().catch(() => ({})) as { error?: string; warning?: string; member?: unknown; magic_link?: string }
    if (!res.ok) {
    ...
    if (d.warning) {
      ...
      setAddToast('✅ Участница добавлена, приглашение отправлено')
    }
    setTimeout(() => setAddToast(''), 10000)
    load()
  }
        ...
        <div style={{
          ...
          {magicLink && (
            <div style={{ marginTop: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 400, marginBottom: 4, opacity: 0.85 }}>
                Ссылка для входа (отправь вручную):
              </div>
              <div style={{
                background: 'rgba(255,255,255,0.15)', borderRadius: 8, padding: '6px 10px',
                fontSize: 11, wordBreak: 'break-all', fontWeight: 400, marginBottom: 6,
              }}>
                {magicLink}
              </div>
              <button
                ...
                style={{
                  ...
                  color: '#fff', fontSize: 12, padding: '4px 10px', cursor: 'pointer', fontWeight: 600,
                }}
              >
                Копировать ссылку
              </button>
              <button
                onClick={() => { setAddToast(''); setMagicLink(''); }}
                style={{
                  background: 'transparent', border: 'none', borderRadius: 6,
                  color: 'rgba(255,255,255,0.6)', fontSize: 12, padding: '4px 10px', cursor: 'pointer', marginLeft: 8,
                }}
              >
                Закрыть
              </button>
                    ...
                    <td style={{ padding: '12px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6 }}>
                          ...
                          {m.role === 'curator' && (
                            <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 100, background: '#E8F5E9', color: '#2E7D50', whiteSpace: 'nowrap' }}>
                              🎓 Куратор
                            </span>
                          )}
                        </div>
