...
function JoinContent() {
  ...
  useEffect(() => {
    ...
    if (params.get('success') === 'true') {
      ...
      if (emailParam) {
        ...
        setStep('otp')
        // OTP уже отправлен вебхуком — просто показываем форму
      }
    }
  ...
  async function handleProceedToPayment(e: React.FormEvent) {
    ...
    const intentParams: any = {
      ...
      failRedirectUrl: 'https://club.nata-tomshina.ru/join?error=true',
      userInfo: {
    ...
    // (CloudPayments doesn't reliably pass custom `data` field to webhook)
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        <Link href="https://nata-tomshina.ru" style={{ textDecoration: 'none' }}>
      ...
      <p style={{ textAlign: 'center', fontSize: 13, color: 'rgba(255,255,255,0.4)', marginTop: 28 }}>
        Уже есть доступ?{' '}
        <Link href="/auth" style={{ color: '#9B7BFF', textDecoration: 'underline' }}>
          Войти →
        </Link>
      </p>
