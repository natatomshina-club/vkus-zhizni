...
import { expandProducts, filterVeggies, normalizeRu, LEGUMES_KEYWORDS, getOffalGroup } from '@/lib/productUtils'
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[weekly/generate] recipes: завтрак=${bfRecipes?.length ?? 0} обед=${mainRecipes?.length ?? 0} салат=${saladCandidates.length} салат_бел=${proteinSaladCandidates.length}`)
    ...
    const offalUsage   = new Map<string, number>()  // group_key → кол-во использований

    // ── Фикс 2: режим субпродуктов ──────────────────────────────────────────
    // Группы субпродуктов из введённых пользователем белков
    const userOffalGroups = new Set<string>()
    for (const p of proteins.offal) {
      const g = getOffalGroup(p)
      if (g) userOffalGroups.add(g)
    }
    const offalMode: 'normal' | 'no_offal' | 'bonus_2' =
      userOffalGroups.size > 0  ? 'normal'   :  // введены свои субпродукты — стандартный режим
      allInputProteins.length >= 3 ? 'no_offal' :  // 3+ белков без субпродуктов — запрет
      'bonus_2'                                     // <3 белков без субпродуктов — до 2 суммарно
    let totalOffalUsage = 0  // суммарно чужих субпродуктов (только в режиме bonus_2)

    // ── Фикс 4: строгий режим белков ────────────────────────────────────────
    const userProteinSet    = new Set(allInputProteins.map(p => normalizeRu(p)))
    const STRICT_PROTEIN_MODE = allInputProteins.length >= 3
    const MAX_EXTRA_PROTEINS  = 2
    const extraProteinsUsed   = new Set<string>()  // чужие protein_tag уже использованные

    // Карта recipe_id → normalizeRu(protein_tag)
    ...
    function trackMealProtein(meal: PlanMeal | null) {
      ...
      const group = getOffalGroup(pt)
      if (group) {
        offalUsage.set(group, (offalUsage.get(group) ?? 0) + 1)
        if (offalMode === 'bonus_2' && !userOffalGroups.has(group)) totalOffalUsage++
      }
      if (STRICT_PROTEIN_MODE && !userProteinSet.has(pt)) extraProteinsUsed.add(pt)
    }

    // ── Пред-фильтры ─────────────────────────────────────────────────────────
    function applyOffalFilter<T extends { id: number; protein_tag?: string | null }>(list: T[]): T[] {
      if (offalMode === 'no_offal') return list.filter(r => !getOffalGroup(r.protein_tag))
      if (offalMode === 'bonus_2' && totalOffalUsage >= MAX_EXTRA_PROTEINS) {
        return list.filter(r => {
          const g = getOffalGroup(r.protein_tag)
          return !g || userOffalGroups.has(g)
        })
      }
      return list
    }

    function applyStrictProteinFilter<T extends { id: number; protein_tag?: string | null }>(list: T[]): T[] {
      if (!STRICT_PROTEIN_MODE || extraProteinsUsed.size < MAX_EXTRA_PROTEINS) return list
      return list.filter(r => {
        const ptNorm = r.protein_tag ? normalizeRu(r.protein_tag) : null
        if (!ptNorm) return true
        return userProteinSet.has(ptNorm) || extraProteinsUsed.has(ptNorm)
      })
    }
    ...
    async function tryBuildMeal(
      ...
      for (const id of candidateIds) {
        ...
        // Фикс 3: исключить бобовые когда дневные углеводы < 70г
        if (memberMacros.carbs < 70) {
          ...
          if (hasLegume) {
            console.log(`[weekly/generate] skip legume recipe id=${id} (daily carbs=${memberMacros.carbs})`)
            continue
          }
        }
    ...
    for (let dayNum = 1; dayNum <= 7; dayNum++) {
      ...
      // ── Завтрак ──
      const bfFiltered = applyStrictProteinFilter(applyOffalFilter(strictVeggieFilter(bfRecipes ?? [])))
      ...
      if (isProteinSaladDay) {
        ...
        const psFiltered = applyStrictProteinFilter(applyOffalFilter(strictVeggieFilter(proteinSaladCandidates)))
        ...
        if (meals_per_day === 3) {
          ...
          const mainFiltered = applyStrictProteinFilter(applyOffalFilter(strictVeggieFilter(mainRecipes ?? [])))
    ...
    const plan_json = { days: allDays, summary }

    console.log(`[weekly/generate] plan done: days=${allDays.length} avg_cal=${summary.avg_calories}`)
